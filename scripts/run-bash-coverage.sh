...
#
# Ported from dotfiles/scripts/run-bash-coverage.sh @ 67417bc (dotfiles
# master, 2026-08-10). Per-repo copy is deliberate — dotfiles, ai-config, and
# math each carry their own; there is no cross-repo sharing mechanism today.
# Whoever ports this again should know what changed here, so it can be
# checked against the new repo's own shape rather than re-derived from
# scratch:
#   1. INCLUDE_FILES predicate — math has no single entry-point script like
#      dotfiles' setup_env.sh. It instruments scripts/*.sh plus every
#      install_deps.sh (both the 11 sub-project top level and each nested
#      <name>-rs/ Rust wrapper), not config/*.sh + lib/*.sh.
#   2. The "at least 1 file" empty-set guard became "at least 0", since math
#      has no guaranteed base file the way dotfiles always has setup_env.sh.
#   3. The printed "Instrumented set:" line and the --list-sources usage text
#      name math's predicate instead of dotfiles'.
#   4. The empty-set error message names math's globs instead of dotfiles'.
#   5. No `_run_coverage` PATH-stripping test helper — this repo's bats suite
#      loads no PATH-based git mock (load_mocks() is opt-in per-test, not
#      global), so tests/scripts/bash_coverage.bats invokes the script
#      directly rather than through a PATH-strip wrapper.
# Everything else — the coverable-line heuristic, the trace filter, the
# union/disagreement computation, the red-suite refusal — is unchanged from
# the source and should be re-copied wholesale on the next defect fix there,
# not patched independently here.
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'USAGE'
...
Inspecting the figure without a full run (all three exit immediately):
  ...
  --list-sources          Print the instrumented set — tracked scripts/*.sh
                          plus every install_deps.sh at both directory-nesting
                          depths (plus the extensionless hooks
                          scripts/pre-push, scripts/pre-commit, and
...
# the reported percentage is unchanged by its absence rather than lowered by
# it. This repo already lived that failure once for LINTING, not coverage:
# the Makefile's SHELL_SOURCES comment records that a 6-file literal list
# covered only scripts/{ci-gate,mutation-classify,rust-check}.sh and the
# three extensionless hooks, missing all 19 install_deps.sh scripts under the
# sub-projects and tests/helpers/common.bash — and the omission was invisible
# in the lint output for exactly this reason. This tracer's predicate is
# derived the same way, so it cannot repeat that specific failure.
#
# The predicate deliberately includes the 19 install_deps.sh scripts even
# though NOTHING under tests/ executes any of them — see the FORBIDDEN note
# in the port's task spec and CLAUDE.md's Bash Coverage section for why:
# excluding an untested majority to raise the percentage is precisely the
# flattering-denominator defect this tooling exists to catch, not a
# legitimate scoping decision. A file is excluded only when it is
# structurally uncoverable (scripts/bash-tracer.sh, below), never because no
# suite happens to reach it yet.
#
# Tracked files, via git — not a filesystem glob. A glob also picks up a
# developer's untracked scratch file under scripts/, which silently joins the
# denominator locally and makes `make bash-coverage` disagree with CI on the
# same commit. It is also what the regression test
# (tests/scripts/bash_coverage.bats) asserts against, so a glob would let the
# two drift apart without either complaining.
INCLUDE_FILES=()
while IFS= read -r _src; do
    [[ -n "${_src}" ]] && INCLUDE_FILES+=("${REPO_ROOT}/${_src}")
done < <(env -u GIT_DIR -u GIT_WORK_TREE -u GIT_COMMON_DIR -u GIT_INDEX_FILE \
    git -C "${REPO_ROOT}" ls-files 'scripts/*.sh' '*/install_deps.sh' '*/*/install_deps.sh' \
    'scripts/pre-push' 'scripts/pre-commit' 'scripts/commit-msg' 2>/dev/null)
...
# Unlike dotfiles (which always has setup_env.sh as a guaranteed base
# member), math has no single entry-point script, so an empty derived set is
# the correct floor to guard rather than "<= 1". Falling back to a glob would
# reintroduce the local-vs-CI drift the comment above the derivation warns
# about, and a silently short instrumented set is exactly this script's
# original defect (dotfiles' 13-of-36 literal-array incident).
if [[ "${#INCLUDE_FILES[@]}" -eq 0 ]]; then
    printf "ERROR: no tracked scripts/*.sh or install_deps.sh found — is this a git checkout?\n" >&2
    exit 1
fi
...
# pitfall. This function and the --check-red-suite flag below close that gap
# without needing a full (multi-minute) suite run to exercise it.
_check_red_suite() {
...
# it was first added between --file-coverage and --check-missing in
# dotfiles, and what the full suite caught there after a filtered run had
# reported green.
...
# like a hang and produces a coverage figure nobody asked for. `--json` is
# the one flag that legally reaches the main run, so it is the only one
# allowed past.
if [[ "${1:-}" == -* && "${1:-}" != "--json" ]]; then
...
# down — CI's `.github/workflows/auto-merge.yml` greps that exact line for
# every integer on it, so a denominator appended there would make the parsed
# percentage multi-line and break the `-lt` comparison.
printf "Instrumented set: tracked scripts/*.sh + install_deps.sh (both depths) + hooks, less bash-tracer.sh (%d files)\n" "${#INCLUDE_FILES[@]}"
