...
| 2026-09-01 | [mutation-notify-attribution](plans/2026-09-01-mutation-notify-attribution.md) | [spec](specs/2026-08-31-mutation-notify-attribution-design.md) | In Progress |
...
| Neither mutation workflow has a `concurrency` block | `grep -n concurrency .github/workflows/mutation-testing*.yml` returns nothing, so two runs of the same workflow can overlap — a `workflow_dispatch` landing on top of the monthly cron, for instance. That is the suspected mechanism behind #99 being filed while #98 was open, and it makes any manual dispatch near a cron window hazardous. A `concurrency: group` with `cancel-in-progress: false` would serialise them; note that cancelling instead would manufacture the terminated-job case the notify job reports on. Found during the 2026-08-31 notify-attribution spec, round 2 risk lens |
| Mutation notify files a duplicate issue instead of commenting | `gh issue list --search 'in:title "..."'` depends on GitHub's issue **search index**; `--label` filtering does not. Measured: #99 was created 2026-08-02T02:13:37Z while #98 — identical title and label — was open since 01:57:03Z and still is. The same lookup run 2026-08-31 returns 98, so the query is not broken; the index is the dependency. Fix is to key on `--label mutation-failure --state open` and filter titles client-side, which is correct whether the 2026-08-02 event was index lag or a race between three overlapping dispatch runs. Affects both `mutation-testing.yml:116` and `mutation-testing-python.yml:119`. Found during the 2026-08-31 notify-attribution spec and deliberately kept out of it — see that spec's Multi-Lens Review, Ergonomics finding 1 |
