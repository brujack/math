...
| `download-artifact` v8 masks a failed download | Both call sites (`mutation-testing.yml:103`, `mutation-testing-python.yml:106`) pair v8's new fail-on-digest-mismatch default with `continue-on-error: true`. The verdict comes from `needs.mutants.result`, so there is no false-clean — but a red run with no artifact then reports "The runner was terminated (exit 143) or the job timed out", naming a cause that did not occur. `action.yml` carries `digest-mismatch` with options `ignore|info|warn|error` and default `error`; `warn` restores the prior behaviour. The alternative is to drop `continue-on-error` and let a real failure be loud — opposite choices, both defensible. First execution under v8 is ~1 September |
