...
| Restore renovate.json's Monday schedule | Set to `at any time` so the first digest-pinning pass could run the night of 2026-08-22 rather than waiting for the Monday window. Restore `["before 5am on monday"]` once that pass is reviewed and merged, so dependency noise stays batched |
